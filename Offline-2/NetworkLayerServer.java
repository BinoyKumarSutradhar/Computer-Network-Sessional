...
public class NetworkLayerServer {
    ...
    public static void main(String[] args) {
        ...
        //mine
        /*for (Router rt1 : routers) {
            for (Integer rt2 : rt1.getNeighborRouterIDs())
            {
        ...
        /*for(int i=0; i<routers.size();i++)
        {
            for(int j=0; j<routers.size();j++)
            {
                ...
                routers.get(i).updateRoutingTable(routers.get(j));
            }
        }*/
    ...
    public static synchronized void DVR(int startingRouterId) {
        ...
        */
        ...
        while (flag)
        {
            ...
            {
                ArrayList<RoutingTableEntry> rte = r.getRoutingTable();
                ArrayList<Integer> nbr_list = r.getNeighborRouterIDs();
                for (Integer n : nbr_list)
                {
                    if(routerMap.containsKey(n))
                    {
                        Router t = routerMap.get(n);
                        if(t.getState() == true)
                        {
                            test = t.sfupdateRoutingTable(r);
                            if(test == true)
                            {
                                x++;
                            }
            ...
            for (int i = startingRouterId-1; i< routers.size(); i++)
            {
                ...
                for (Integer n : nbr_list)
                {
                    ...
                    {
                        Router t = routerMap.get(n);
                        if(t.getState() == true)// state check korar karone problem hoye thakte pare
                        {
                            test = t.sfupdateRoutingTable(r);
                            if(test == true)
                            {
